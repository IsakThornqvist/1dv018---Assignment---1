...
The main benefit with this approach is that it avoids a lot of unecessary and impossible combinations and because of that the algorithm works faster than the brute force approach which basically checks every possible combination and by doing that wastes time.
...
In my experiments bubble sort was a lot slower than the selection and insertion sort while the other two end up really close to each other in execution time. From the examples I looked at to compare with it seems like the bubble sort is slower than the other two but I did not expect such a big difference. The difference was larger than I exoected especially when compared to insertion and selection sort.
